...
- **例程姿势（2026-09-05 定，后修正）**：**探索/学新概念默认走 `go run` 单文件例程看输出**（保持原工作流），纪律只有一条：**一个文件夹最多一个 `func main`**（目录 = 包，多 main 会触发 gopls redeclared 红波浪；不影响 go run file.go）。**已学透的结论**可选固化进 `verify/verify_test.go` 断言（`go test ./verify/` 自动判对错 + 回归：语义变化会红）——**是回归保险，不是默认流程**
